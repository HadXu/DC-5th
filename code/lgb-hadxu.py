...
# line['region_encode'] = line['region_encode'].astype('category')
line['O_LINENO'] = line['O_LINENO'].astype('category')
...
def get_stacking(X_train, labels, n_folds=10):
    ...
    for n_fold,(train_idx, val_idx) in enumerate(folds.split(labels)):
        ...
        noice = np.random.randint(-2, 2, train_y.shape)
        train_y = train_y + noice
...
#                           learning_rates=lambda x:0.08 if x>15000 else 0.1, 
                          valid_sets=lgb_eval,
                          verbose_eval=50)
    ...
    "---save---"
    with open('test_nfolds_sets_gbm_0814_noice_2.txt', 'wb') as data_file:
...
"---save---"
with open('meta_train_lgb_0812.txt', 'wb') as data_file:
    pickle.dump(meta_train, data_file)
with open('meta_test_lgb_0812.txt', 'wb') as data_file:
