...
def get_stacking(X_train, labels, n_folds=10):
    ...
    clf = CatBoostRegressor(verbose=True, depth=8, iterations=2300, learning_rate=0.95,
                   loss_function='RMSE', bagging_temperature=0.8,
                   l2_leaf_reg=4,
                   rsm=0.8, 
                   thread_count=16,
                   random_seed=2018)
    ...
    for n_fold,(train_idx, val_idx) in enumerate(folds.split(labels)):
        ...
        print('fold',n_fold)
        model = clf.fit(train, train_y, eval_set=[(val, val_y)])

        second_level_train_set[val_idx] = model.predict(val)
        test_nfolds_sets[:,n_fold] = model.predict(X_test)
    
    "---save---"
    with open('test_nfolds_sets_cat_0808.txt', 'wb') as data_file:
...
"---save---"
with open('meta_train_cat_0808.txt', 'wb') as data_file:
    pickle.dump(meta_train, data_file)
with open('meta_test_cat_0808.txt', 'wb') as data_file:
