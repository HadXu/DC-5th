...
def get_stacking(X_train, labels, n_folds=10):
    ...
    model = xgb.sklearn.XGBRegressor(nthread=10, n_estimators=100, max_depth=13, seed=2018,
                                 learning_rate=0.5, subsample=0.7,
                                 min_child_weight=6, colsample_bytree=0.75, gamma=0.3,
                                 reg_alpha=0.19, reg_lambda=0.26, silent=False)
    ...
    for n_fold,(train_idx, val_idx) in enumerate(folds.split(labels)):
        ...
        print('fold',n_fold)
        clf = model.fit(train, train_y, eval_set=[(val, val_y)], eval_metric='rmse', early_stopping_rounds=100)
